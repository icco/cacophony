module github.com/icco/cacophony

require (
	github.com/99designs/gqlgen v0.7.2 // indirect
	github.com/GuiaBolso/darwin v0.0.0-20170210191649-86919dfcf808
	github.com/aws/aws-sdk-go v1.16.27 // indirect
	github.com/coreos/go-systemd v0.0.0-20190204112023-081494f7ee4f // indirect
	github.com/coreos/pkg v0.0.0-20180928190104-399ea9e2e55f
	github.com/dghubble/go-twitter v0.0.0-20190108053744-7fd79e2bcc65
	github.com/dghubble/oauth1 v0.5.0
	github.com/felixge/httpsnoop v1.0.0
	github.com/gofrs/uuid v3.2.0+incompatible // indirect
	github.com/hellofresh/logging-go v0.3.0
	github.com/icco/cron v0.0.0-20190121032237-b4835d3918f3
	github.com/icco/graphql v0.0.0-20190203211859-7ccb702358d8 // indirect
	github.com/icco/logrus-stackdriver-formatter v0.3.0
	github.com/lib/pq v1.0.0
	github.com/shurcooL/home v0.0.0-20190204141146-5c8ae21d4240 // indirect
	github.com/sirupsen/logrus v1.3.0
	golang.org/x/build v0.0.0-20190205044948-0fbf6af331f6 // indirect
	golang.org/x/sys v0.0.0-20190204203706-41f3e6584952 // indirect
	golang.org/x/tools v0.0.0-20190204220159-b2f7fe607dec // indirect
)
